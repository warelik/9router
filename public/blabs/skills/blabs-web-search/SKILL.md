---
name: blabs-web-search
description: Web search via BLabsAIGate /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---

# BLabsAIGate — Web Search
...
```bash
curl $BLABS_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$BLABS_URL/v1/models/info?id=tavily/search"
```
...
```bash
curl -X POST $BLABS_URL/v1/search \
  -H "Authorization: Bearer $BLABS_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"tavily","query":"BLabsAIGate open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.BLABS_URL}/v1/search`, {
  method: "POST",
