---
name: blabs-chat
description: Chat / code generation via BLabsAIGate using OpenAI /v1/chat/completions or Anthropic /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through BLabsAIGate.
---
...
- `POST $BLABS_URL/v1/chat/completions` — OpenAI format
- `POST $BLABS_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $BLABS_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$BLABS_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $BLABS_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.BLABS_URL}/v1`, apiKey: process.env.BLABS_KEY });
...
```bash
curl -X POST $BLABS_URL/v1/messages \
  -H "Authorization: Bearer $BLABS_KEY" \
