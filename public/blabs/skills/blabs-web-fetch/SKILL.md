---
name: blabs-web-fetch
description: Fetch URL → markdown / text / HTML via BLabsAIGate /v1/web/fetch using Firecrawl / Jina Reader / Tavily Extract / Exa Contents. Use when the user wants to scrape a webpage, extract URL content, read article, or convert a URL to markdown.
---

# BLabsAIGate — Web Fetch
...
```bash
curl $BLABS_URL/v1/models/web | jq '.data[] | select(.kind=="webFetch") | .id'
# Per-provider params
curl "$BLABS_URL/v1/models/info?id=firecrawl/fetch"
```
...
### Jina Reader
```bash
curl -X POST $BLABS_URL/v1/web/fetch \
  -H "Authorization: Bearer $BLABS_KEY" \
  -H "Content-Type: application/json" \
  -d '{"model":"jina-reader","url":"this showroom","format":"markdown"}'
```
...
```js
const r = await fetch(`${process.env.BLABS_URL}/v1/web/fetch`, {
  method: "POST",
